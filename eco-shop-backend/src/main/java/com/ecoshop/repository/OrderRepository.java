...
public interface OrderRepository extends JpaRepository<Order, Long> {
    ...
    @Query("SELECT COALESCE(SUM(o.totalCarbonFootprint), 0) FROM Order o WHERE o.userId = :userId")
    ...
    @Query(value = "SELECT FORMATDATETIME(o.created_at, 'MMM') as mon, COALESCE(SUM(o.total_carbon_footprint), 0) " +
           "FROM orders o WHERE o.user_id = :userId GROUP BY FORMATDATETIME(o.created_at, 'MMM'), MONTH(o.created_at) " +
           "ORDER BY MONTH(o.created_at)", nativeQuery = true)
    List<Object[]> monthlyCarbonTrend(@Param("userId") Long userId);

    @Query("SELECT o.userId, SUM(o.totalCarbonFootprint) as total FROM Order o GROUP BY o.userId ORDER BY total DESC")
    List<Object[]> carbonRanking();

    @Query("SELECT COALESCE(SUM(o.totalCarbonFootprint), 0) FROM Order o")
